...
3. Run the **Upload files to Figshare draft** workflow from GitHub Actions. It targets only item `33472651` and uploads the existing manuscript PDF plus a ZIP from the workflow's exact git commit. It checks the title and license, verifies file sizes and MD5 checksums, and skips already-complete identical files on retries. It does not delete files, change metadata, or publish.
